...
export function TaskCard({ task, column, onUpdateTask, onRemoveTask }) {
  ...
  useEffect(() => {
    if (task.isEditing) setEditing(true)
  }, [task.isEditing])

  function handleSave() {
    onUpdateTask?.(column, task.id, { title: title || "Untitled", description, isEditing: false })
    setEditing(false)
  }
          ...
          <CardContent className="p-3 space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="icon-xs" variant="ghost" onClick={handleDelete} aria-label="Excluir task">
                  <Trash2 size={14} />
                </Button>
              </div>
            </div>
